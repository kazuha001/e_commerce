    ...
    if ($check_result->num_rows > 0) {
        ...
        if ($trans_result->num_rows > 0) {
            ...
            if (isset($_POST["cancel"])) {
                $set_reverse = "Carts";
                $stmt2 = $conn->prepare("UPDATE trans SET process = ? WHERE user_id = ?");
                $stmt2->bind_param("si", $set_reverse, $trans_acc["user_id"]);
                $stmt2->execute();
                echo '
            ...
            $acc = $conn->prepare("UPDATE user_accounts SET request_order = ? WHERE id = ?");
            $acc->bind_param("si", $set2, $trans_acc["user_id"]);
            $acc->execute();
